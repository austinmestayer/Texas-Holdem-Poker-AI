...
class RiskReward(Strategy):
    ...
    def decide_play(self, player, pot):
        hand_types = ["High Card", "Pair", "Two Pair", "Triple", "Straight", "Flush", "Full House", "Four",
        ...
        risk = self.calc_risk(player)

        print("Me code: ", probBetterHand)
        print(rep)
        player.print_cards()
        if (probBetterHand * probToWin) - risk < -0.1:
            player.fold(pot)
        elif probToWin > 0.95:
            player.bet(pot, player.stack / 10)
        else:
            player.check_call(pot)
    ...
    def cardsNeededForStraight(self, hand):
        ...
        if abs(hand[0].value - hand[1].value) < 5:
            ...
            dupeCard = []
            while i < (len(hand) - 2):
                if abs(hand[i + 2].value - hand[0].value) < 5 and abs(hand[i + 2].value - hand[1].value) < 5 and hand[
                    i + 2].value not in dupeCard:
